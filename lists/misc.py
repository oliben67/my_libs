...
def insert_after(lst, sub, elem, _all=False):
    ...
    split_lst =  [list(group if not k else []) for k, group in groupby(lst, lambda x: x==elem)]
    return flatten([sub_list + [elem] + sub  if not sub_list else sub_list for sub_list in split_lst])
...
def insert_before(lst, sub, elem, _all=False):
    ...
    split_lst =  [list(group if not k else []) for k, group in groupby(lst, lambda x: x==elem)]
    return flatten([sub + [elem] + sub_list if not sub_list else sub_list for sub_list in split_lst])